...
fn oracle_prop_min_basics() {
    if !oracle_prop_enabled() {
        tracing::info!("skipping oracle_prop_min_basics: set NEOVM_ENABLE_ORACLE_PROPTEST=1");
        return;
    }
...
fn oracle_prop_min_error_cases() {
    if !oracle_prop_enabled() {
        tracing::info!("skipping oracle_prop_min_error_cases: set NEOVM_ENABLE_ORACLE_PROPTEST=1");
        return;
    }
