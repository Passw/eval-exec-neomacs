...
pub(crate) fn terminal_designator_eval_p(
    ...
    terminal_designator_p(value)
        || crate::elisp::display::live_frame_designator_p(eval, value)
}
...
pub(crate) fn builtin_frame_terminal(args: Vec<Value>) -> EvalResult {
    ...
    if let Some(frame) = args.first() {
        crate::elisp::display::expect_frame_designator(frame)?;
    }
    Ok(terminal_handle_value())
}
...
pub(crate) fn builtin_frame_terminal_eval(
    ...
    if let Some(frame) = args.first() {
        if !frame.is_nil()
            && !crate::elisp::display::live_frame_designator_p(eval, frame)
        {
            return Err(signal(
                "wrong-type-argument",
