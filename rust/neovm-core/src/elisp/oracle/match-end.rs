...
fn oracle_prop_match_end_basics() {
    if !oracle_prop_enabled() {
        eprintln!("skipping oracle_prop_match_end_basics: set NEOVM_ENABLE_ORACLE_PROPTEST=1");
        return;
    }
...
fn oracle_prop_match_end_wrong_type_error() {
    if !oracle_prop_enabled() {
        eprintln!("skipping oracle_prop_match_end_wrong_type_error: set NEOVM_ENABLE_ORACLE_PROPTEST=1");
        return;
    }
