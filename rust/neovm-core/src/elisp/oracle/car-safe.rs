...
fn oracle_prop_car_safe_basics() {
    if !oracle_prop_enabled() {
        eprintln!("skipping oracle_prop_car_safe_basics: set NEOVM_ENABLE_ORACLE_PROPTEST=1");
        return;
    }
...
fn oracle_prop_car_safe_wrong_arity_error() {
    if !oracle_prop_enabled() {
        eprintln!("skipping oracle_prop_car_safe_wrong_arity_error: set NEOVM_ENABLE_ORACLE_PROPTEST=1");
        return;
    }
