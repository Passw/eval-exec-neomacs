...
fn oracle_prop_set_buffer_basics() {
    if !oracle_prop_enabled() {
        eprintln!("skipping oracle_prop_set_buffer_basics: set NEOVM_ENABLE_ORACLE_PROPTEST=1");
        return;
    }
...
fn oracle_prop_set_buffer_error_cases() {
    if !oracle_prop_enabled() {
        eprintln!("skipping oracle_prop_set_buffer_error_cases: set NEOVM_ENABLE_ORACLE_PROPTEST=1");
        return;
    }
