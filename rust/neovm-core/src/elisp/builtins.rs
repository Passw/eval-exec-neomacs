...
fn maybe_mark_pcase_fallback_materialized(
    ...
    let _ = builtin_put(
        eval,
        vec![Value::symbol(name), Value::symbol(property), Value::True],
    );
}
...
mod tests {
    ...
    fn symbol_function_resolves_builtin_and_special_names() {
        ...
        assert!(crate::elisp::autoload::is_autoload_value(
            &pcase_dolist_macro
        ));
    ...
    #[test]
    fn func_arity_eval_pcase_dolist_followup_indirect_arity_matches_oracle() {
        let mut eval = crate::elisp::eval::Evaluator::new();

        let startup_indirect =
            builtin_indirect_function(&mut eval, vec![Value::symbol("pcase-dolist")])
                .expect("indirect-function should resolve pcase-dolist startup shape");
        assert!(crate::elisp::autoload::is_autoload_value(&startup_indirect));
        let startup_indirect_arity = builtin_func_arity_eval(&mut eval, vec![startup_indirect])
        ...
        let symbol_arity = builtin_func_arity_eval(&mut eval, vec![Value::symbol("pcase-dolist")])
            .expect("func-arity should resolve pcase-dolist symbol");
        assert_eq!(
            symbol_arity,
            Value::cons(Value::Int(1), Value::symbol("many"))
        );
        ...
        let materialized_indirect =
            builtin_indirect_function(&mut eval, vec![Value::symbol("pcase-dolist")])
                .expect("indirect-function should resolve pcase-dolist after func-arity");
        assert!(
            !crate::elisp::autoload::is_autoload_value(&materialized_indirect),
            "indirect-function should stop returning startup autoload after func-arity symbol"
        );
    ...
    fn func_arity_eval_pcase_let_family_followup_indirect_arity_matches_oracle() {
        ...
        for name in ["pcase-let", "pcase-let*"] {
            let startup_indirect = builtin_indirect_function(&mut eval, vec![Value::symbol(name)])
                .unwrap_or_else(|_| {
                    panic!("indirect-function should resolve {name} startup shape")
                });
            assert!(
            ...
            let materialized_indirect =
                builtin_indirect_function(&mut eval, vec![Value::symbol(name)]).unwrap_or_else(
                    |_| panic!("indirect-function should resolve {name} after func-arity"),
                );
            assert!(
                !crate::elisp::autoload::is_autoload_value(&materialized_indirect),
            ...
            let followup_arity = builtin_func_arity_eval(&mut eval, vec![materialized_indirect])
                .unwrap_or_else(|_| {
                    panic!("materialized indirect {name} should satisfy func-arity")
                });
            assert_eq!(
                followup_arity,
                Value::cons(Value::Int(1), Value::symbol("many"))
    ...
    fn macroexpand_runtime_expands_global_and_nested_macros() {
        ...
        let pcase_let_mixed_forms = crate::elisp::parser::parse_forms(
            r#"(pcase-let ((x 1) (`(,a ,b) '(2 3))) (list x a b))"#,
        )
        .expect("mixed pcase-let form should parse");
        let pcase_let_mixed = crate::elisp::eval::quote_to_value(
            pcase_let_mixed_forms
                .first()
                .expect("mixed pcase-let parse should yield one form"),
        );
        let pcase_let_mixed_expanded = builtin_macroexpand_eval(&mut eval, vec![pcase_let_mixed])
            .expect("macroexpand should lower mixed pcase-let bindings");
        let pcase_let_mixed_expected_forms = crate::elisp::parser::parse_forms(
            r#"(let ((x 1) (x1 '(2 3))) (pcase-let* ((`(,a ,b) x1)) (list x a b)))"#,
        )
        .expect("mixed pcase-let expected expansion should parse");
        let pcase_let_mixed_expected = crate::elisp::eval::quote_to_value(
            pcase_let_mixed_expected_forms
                .first()
        ...
        let pcase_let_star_mixed_forms = crate::elisp::parser::parse_forms(
            r#"(pcase-let* ((`(,a ,b) '(1 2)) (z (+ a b))) (list z a b))"#,
        )
        .expect("mixed pcase-let* form should parse");
        let pcase_let_star_mixed = crate::elisp::eval::quote_to_value(
            pcase_let_star_mixed_forms
                .first()
                .expect("mixed pcase-let* parse should yield one form"),
        );
        let pcase_let_star_mixed_expanded =
            builtin_macroexpand_eval(&mut eval, vec![pcase_let_star_mixed])
                .expect("macroexpand should lower simple mixed pcase-let* bindings");
        let pcase_let_star_mixed_expected_forms = crate::elisp::parser::parse_forms(
            r#"(progn (ignore (consp '(1 2))) (let* ((x0 (car-safe '(1 2))) (x1 (cdr-safe '(1 2)))) (progn (ignore (consp x1)) (let* ((x2 (car-safe x1)) (x3 (cdr-safe x1))) (progn (ignore (null x3)) (let ((a x0) (b x2)) (let* ((z (+ a b))) (list z a b))))))))"#,
        )
        .expect("mixed pcase-let* expected expansion should parse");
        let pcase_let_star_mixed_expected = crate::elisp::eval::quote_to_value(
            pcase_let_star_mixed_expected_forms
                .first()
                .expect("mixed pcase-let* expected parse should yield one form"),
        );
        ...
        let pcase_let_star_mixed_dotted_forms =
            crate::elisp::parser::parse_forms(r#"(pcase-let* ((`(,a . ,b) '(1 2 3))) (list a b))"#)
        ...
        let pcase_let_star_vector_pattern_forms =
            crate::elisp::parser::parse_forms(r#"(pcase-let* ((`[,a ,b] [1 2])) (list a b))"#)
        ...
        let pcase_let_star_no_body_symbol = builtin_macroexpand_eval(
            &mut eval,
            vec![Value::list(vec![
                Value::symbol("pcase-let*"),
        ...
        assert_eq!(pcase_let_star_no_body_symbol, Value::Nil);
        let pcase_let_star_no_body_pattern = builtin_macroexpand_eval(
            &mut eval,
            vec![Value::list(vec![
                Value::symbol("pcase-let*"),
                Value::list(vec![Value::list(vec![
        ...
        .expect("macroexpand should collapse no-body pcase-let* pattern bindings to nil");
        assert_eq!(pcase_let_star_no_body_pattern, Value::Nil);
        let pcase_let_star_no_body_nonlist = builtin_macroexpand_eval(
            &mut eval,
            vec![Value::list(vec![
                Value::symbol("pcase-let*"),
                Value::symbol("x"),
        ...
        let bad_pcase_let_binding_tail = builtin_macroexpand_eval(
            &mut eval,
            vec![Value::list(vec![
                Value::symbol("pcase-let"),
                Value::list(vec![
        ...
        let pcase_let_symbol_extra_forms = builtin_macroexpand_eval(
            &mut eval,
            vec![Value::list(vec![
                Value::symbol("pcase-let"),
                Value::list(vec![Value::list(vec![
        ...
        .expect("macroexpand should preserve let-style symbol binding forms");
        assert_eq!(
            pcase_let_symbol_extra_forms,
            Value::list(vec![
                Value::symbol("let*"),
                Value::list(vec![Value::list(vec![
                    Value::symbol("x"),
        ...
        let bad_pcase_let_star_unknown_pattern = builtin_macroexpand_eval(
            &mut eval,
            vec![Value::list(vec![
                Value::symbol("pcase-let*"),
                Value::list(vec![
                    ...
                    Value::list(vec![Value::symbol("z"), Value::Int(2)]),
                ]),
                Value::symbol("z"),
            ])],
        )
        ...
        let bad_pcase_let_star_unknown_pattern_tail = builtin_macroexpand_eval(
            &mut eval,
            vec![Value::list(vec![
                Value::symbol("pcase-let*"),
                Value::list(vec![
                    Value::list(vec![Value::cons(Value::symbol("x"), Value::symbol("y")), Value::Int(1)]),
                    Value::symbol("q"),
                ]),
                Value::symbol("r"),
            ])],
