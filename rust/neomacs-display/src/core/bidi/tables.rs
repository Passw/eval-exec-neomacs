...
mod tests {
    ...
    #[test]
    fn test_cjk() {
        assert_eq!(bidi_class('\u{4E00}'), BidiClass::L); // CJK unified
        assert_eq!(bidi_class('\u{3042}'), BidiClass::L); // Hiragana
    }
    ...
    fn test_bracket_type() {
        assert!(matches!(bracket_type('('), BracketType::Open(')')));
        assert!(matches!(bracket_type(')'), BracketType::Close('(')));
        assert!(matches!(bracket_type('['), BracketType::Open(']')));
        assert!(matches!(bracket_type('A'), BracketType::None));
    }
}
