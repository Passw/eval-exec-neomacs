...
fn linux_ci_setup_profiles_expose_capabilities_and_reject_unknown_profiles() {
    ...
    // Every profile setup-linux.sh defines. `--list` exits non-zero on an
    // unknown profile, so this list and the script's case arms must agree.
    for profile in ["build", "oracle", "ecosystem", "release"] {
        assert!(
            !packages(profile)
                .lines()
