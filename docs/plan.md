...
- Closed startup `variable-documentation` string-property parity with a 602-symbol oracle-backed batch (round 35):
  - runtime changes:
    ...
    - expanded `STARTUP_VARIABLE_DOC_STRING_PROPERTIES` with the remaining startup string-doc symbols, including:
      - `process-*`, `query-replace-*`, `read-*`, `regexp-*`, `safe-local-*`, `search-*`, `selection-*`, `show-paren-*`, `tab-bar-*`, `tabulated-list-*`, `tool-bar-*`, `touch-screen-*`, `tramp-*`, `undo-*`, `uniquify-*`, `user-*`, `vc-*`, `window-*`, `x-dnd-*`, and `yank-*`.
    - `rust/neovm-core/src/elisp/eval.rs`
    - expanded `startup_string_variable_docs_are_seeded_at_startup` with additional representative lock-ins (`process-file-side-effects`, `process-menu-mode-map`, `prog-mode-map`, `query-about-changed-file`, `read-extended-command-predicate`, `regexp-search-ring-max`, `safe-local-variable-values`, `selection-coding-system`, `show-paren-mode`, `tab-bar-format`, `tool-bar-map`, `transient-mark-mode-hook`, `user-emacs-directory`, `window-size-fixed`, `yank-transform-functions`) and integer-doc absence checks.
  - oracle corpus changes:
    - `test/neovm/vm-compat/cases/documentation-property-semantics.{forms,expected.tsv}`
    - added one startup `get`-property matrix row covering the same 602-symbol batch (`(integerp/get, stringp/get)` parity).
  - parity movement:
    - startup `variable-documentation` type counts now:
      - GNU Emacs oracle: `(integer=761, string=1904)`
      - NeoVM: `(integer=761, string=1904)` (startup string-doc parity reached).
  - verified:
    ...
    - `make -C test/neovm/vm-compat check-one-neovm CASE=documentation-property-semantics` (pass, 55/55)
...
- Expanded startup `variable-documentation` string-property seeding with a 200-symbol oracle-backed batch (round 31):
  - runtime changes:
    - `rust/neovm-core/src/elisp/doc.rs`
    - expanded `STARTUP_VARIABLE_DOC_STRING_PROPERTIES` with 200 additional non-integer startup symbols, including:
      - `buffer-*`, `buffers-menu-*`, `button-*`, `command-line-*`, `comment-*`, `completion-*`, `completions-*`, `context-menu-*`, `current-input-method`, `custom-*`, and `default-input-method`.
    - `rust/neovm-core/src/elisp/eval.rs`
    - expanded `startup_string_variable_docs_are_seeded_at_startup` with additional startup lock-ins (`buffer-quit-function`, `command-line-functions`, `comment-start`, `completion-styles`, `context-menu-mode`, `current-input-method`, `custom-enabled-themes`, `default-input-method`) and integer-doc absence checks for the same symbols.
  - oracle corpus changes:
    - `test/neovm/vm-compat/cases/documentation-property-semantics.{forms,expected.tsv}`
    - added a second startup `get`-property matrix row covering this 200-symbol batch (`(integerp/get, stringp/get)` parity).
  - parity movement:
    - startup `variable-documentation` type counts now:
      - GNU Emacs oracle: `(integer=761, string=1904)`
      - NeoVM: `(integer=761, string=352)` (improved from `string=152`).
  - verified:
    - `cargo test --manifest-path rust/neovm-core/Cargo.toml startup_string_variable_docs_are_seeded_at_startup` (pass)
    - `cargo test --manifest-path rust/neovm-core/Cargo.toml documentation_property_eval_` (pass; 28 tests)
    - `make -C test/neovm/vm-compat record FORMS=cases/documentation-property-semantics.forms EXPECTED=cases/documentation-property-semantics.expected.tsv` (pass)
    - `make -C test/neovm/vm-compat check-one-neovm CASE=documentation-property-semantics` (pass, 51/51)
    - `make -C test/neovm/vm-compat check-startup-doc-stub-coverage` (pass; missing=0, extra=0)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded startup `documentation-property` integer-offset coverage for core editor variables (batch round 27):
  - runtime changes:
    - `rust/neovm-core/src/elisp/doc.rs`
    - `startup_variable_doc_offset_symbol` now derives eligibility from `startup_variable_doc_stub`, preventing drift between the offset gate and stub table.
    - added startup variable doc stubs for:
      - `abbrev-mode`, `after-change-functions`, `before-change-functions`, `auto-fill-function`, `case-fold-search`, `buffer-file-name`, `buffer-undo-list`, `fill-column`, `current-prefix-arg`, `this-command`, `undo-limit`, `undo-strong-limit`, `select-active-regions`, `transient-mark-mode`, `exec-path`, `shell-file-name`, `tool-bar-mode`, `tab-bar-mode`, `display-line-numbers`, `truncate-lines`, `word-wrap`, `user-full-name`, `window-system`, `completion-ignore-case`, `comment-end-can-be-escaped`, `debug-on-quit`, `major-mode`, `mode-line-format`
    - added unit lock-in:
      - `documentation_property_eval_case_fold_search_integer_property_returns_string`
    - `rust/neovm-core/src/elisp/eval.rs`
    - startup now seeds integer `variable-documentation` offsets for the same core variable set so evaluator/runtime paths resolve these docs in parity checks.
  - oracle corpus changes:
    - `test/neovm/vm-compat/cases/documentation-property-semantics.{forms,expected.tsv}`
    - added first-line prefix parity matrix for the above core variable set.
  - verified:
    - `cargo test --manifest-path rust/neovm-core/Cargo.toml documentation_property_eval_` (pass; 12 tests)
    - `make -C test/neovm/vm-compat record FORMS=cases/documentation-property-semantics.forms EXPECTED=cases/documentation-property-semantics.expected.tsv` (pass)
    - `make -C test/neovm/vm-compat check-one-neovm CASE=documentation-property-semantics` (pass, 22/22)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded coding-system runtime/designator error lock-ins (batch round 13):
  - oracle corpus changes:
    ...
    - `test/neovm/vm-compat/cases/read-batch-error-payloads.{forms,expected.tsv}`
    - added alias/designator type-error and priority-argument probes plus dotted/vector unread-queue payload coverage for coding readers.
  - verified:
    - `make -C test/neovm/vm-compat check-neovm-list LIST=<batch-list>` (pass; 18/18, 38/38, 16/16, 56/56)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded remaining vector-vector non-char queue tails (batch round 6):
  - oracle corpus changes:
    ...
    - `test/neovm/vm-compat/cases/read-key-sequence-vector-vector-tail-non-char-semantics.{forms,expected.tsv}`
    - `test/neovm/vm-compat/cases/read-key-vector-vector-tail-non-character-tail-semantics.{forms,expected.tsv}`
    - added one deeper-tail probe per case.
  - verified:
    - `make -C test/neovm/vm-compat check-neovm-list LIST=<batch-list>` (pass; 10/10, 10/10, 11/11)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-event` / `read-key` tail lock-ins with deeper payload depth (batch round 4):
  - oracle corpus changes:
    ...
    - `test/neovm/vm-compat/cases/read-event-non-character-tail-semantics.{forms,expected.tsv}`
    - `test/neovm/vm-compat/cases/read-event-symbol-tail-semantics.{forms,expected.tsv}`
    - `test/neovm/vm-compat/cases/read-event-symbol-vector-tail-semantics.{forms,expected.tsv}`
    - `test/neovm/vm-compat/cases/read-event-vector-symbol-tail-semantics.{forms,expected.tsv}`
    - `test/neovm/vm-compat/cases/read-event-vector-tail-semantics.{forms,expected.tsv}`
    - `test/neovm/vm-compat/cases/read-event-vector-vector-tail-semantics.{forms,expected.tsv}`
    - `test/neovm/vm-compat/cases/read-key-non-character-tail-semantics.{forms,expected.tsv}`
    - `test/neovm/vm-compat/cases/read-key-symbol-tail-semantics.{forms,expected.tsv}`
    - `test/neovm/vm-compat/cases/read-key-symbol-vector-tail-semantics.{forms,expected.tsv}`
    - `test/neovm/vm-compat/cases/read-key-vector-symbol-tail-semantics.{forms,expected.tsv}`
    - `test/neovm/vm-compat/cases/read-key-vector-tail-semantics.{forms,expected.tsv}`
    - `test/neovm/vm-compat/cases/read-key-vector-vector-tail-non-character-tail-semantics.{forms,expected.tsv}`
    - `test/neovm/vm-compat/cases/read-key-vector-vector-tail-semantics.{forms,expected.tsv}`
    - added one deeper-tail probe per case (extra scalar/symbol/vector tail element).
  - verified:
    - `make -C test/neovm/vm-compat check-neovm-list LIST=<batch-list>` (pass; updated cases now 10/10, `read-event-list-tail-semantics` 11/11)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-char-exclusive` vector-vector tail lock-ins with deeper payload depth (round 3):
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list [foo] 98 [bar] [baz] [qux] [quux] [quuz])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-char-exclusive-vector-vector-tail-semantics` (pass, 7/7)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-char-exclusive` symbol-vector tail lock-ins with deeper payload depth (round 3):
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 'foo 98 [bar] [baz] [qux] [quux] [quuz])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-char-exclusive-symbol-vector-tail-semantics` (pass, 7/7)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-char-exclusive` vector-symbol tail lock-ins with deeper payload depth (round 3):
  - oracle corpus changes:
...
- Expanded `read-char-exclusive` symbol tail lock-ins with deeper payload depth (round 3):
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 'foo 98 'bar 'baz 'qux 'quux 'quuz)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-char-exclusive-symbol-tail-semantics` (pass, 7/7)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-char-exclusive` non-character tail lock-ins with deeper payload depth (round 3):
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 1.0 98 97 96 95 94 93)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-char-exclusive-non-character-tail-semantics` (pass, 7/7)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-char` non-character-vector tail lock-ins with deeper payload depth (round 3):
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list [1 2] 98 97 96 95 94 93)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-char-non-character-vector-tail-semantics` (pass, 7/7)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-char` symbol tail lock-ins with deeper payload depth (round 3):
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 'foo 98 'bar 'baz 'qux 'quux 'quuz)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-char-symbol-tail-semantics` (pass, 7/7)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-char` non-character tail lock-ins with deeper payload depth (round 3):
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 1.0 98 97 96 95 94 93)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-char-non-character-tail-semantics` (pass, 7/7)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-char` vector-vector tail lock-ins with deeper payload depth (round 3):
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list [foo] 98 [bar] [baz] [qux] [quux] [quuz])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-char-vector-vector-tail-semantics` (pass, 7/7)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-char` vector-symbol tail lock-ins with deeper payload depth (round 3):
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list [foo] 98 'bar 'baz 'qux 'quux 'quuz)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-char-vector-symbol-tail-semantics` (pass, 7/7)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-char-exclusive` symbol-vector tail lock-ins with deeper payload depth (round 2):
  - oracle corpus changes:
    - `test/neovm/vm-compat/cases/read-char-exclusive-symbol-vector-tail-semantics.forms`
    - `test/neovm/vm-compat/cases/read-char-exclusive-symbol-vector-tail-semantics.expected.tsv`
    - added deeper-tail probe `(list 'foo 98 [bar] [baz] [qux] [quux])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-char-exclusive-symbol-vector-tail-semantics` (pass, 6/6)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-char-exclusive` vector-vector tail lock-ins with deeper payload depth (round 2):
  - oracle corpus changes:
    - `test/neovm/vm-compat/cases/read-char-exclusive-vector-vector-tail-semantics.forms`
    - `test/neovm/vm-compat/cases/read-char-exclusive-vector-vector-tail-semantics.expected.tsv`
    - added deeper-tail probe `(list [foo] 98 [bar] [baz] [qux] [quux])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-char-exclusive-vector-vector-tail-semantics` (pass, 6/6)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-char-exclusive` vector-symbol tail lock-ins with deeper payload depth (round 2):
  - oracle corpus changes:
    - `test/neovm/vm-compat/cases/read-char-exclusive-vector-symbol-tail-semantics.forms`
    - `test/neovm/vm-compat/cases/read-char-exclusive-vector-symbol-tail-semantics.expected.tsv`
    - added deeper-tail probe `(list [foo] 98 'bar 'baz 'qux 'quux)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-char-exclusive-vector-symbol-tail-semantics` (pass, 6/6)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-char-exclusive` non-character tail lock-ins with deeper payload depth (round 2):
  - oracle corpus changes:
    - `test/neovm/vm-compat/cases/read-char-exclusive-non-character-tail-semantics.forms`
    - `test/neovm/vm-compat/cases/read-char-exclusive-non-character-tail-semantics.expected.tsv`
    - added deeper-tail probe `(list 1.0 98 97 96 95 94)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-char-exclusive-non-character-tail-semantics` (pass, 6/6)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-char-exclusive` symbol tail lock-ins with deeper payload depth (round 2):
  - oracle corpus changes:
    - `test/neovm/vm-compat/cases/read-char-exclusive-symbol-tail-semantics.forms`
    - `test/neovm/vm-compat/cases/read-char-exclusive-symbol-tail-semantics.expected.tsv`
    - added deeper-tail probe `(list 'foo 98 'bar 'baz 'qux 'quux)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-char-exclusive-symbol-tail-semantics` (pass, 6/6)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-char-exclusive` vector tail lock-ins with deeper payload depth (round 2):
  - oracle corpus changes:
    - `test/neovm/vm-compat/cases/read-char-exclusive-vector-tail-semantics.forms`
    - `test/neovm/vm-compat/cases/read-char-exclusive-vector-tail-semantics.expected.tsv`
    - added deeper-tail probe `(list 97 98 99 100 101 [mouse-1])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-char-exclusive-vector-tail-semantics` (pass, 6/6)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-char` non-character-vector tail lock-ins with deeper payload depth (round 2):
  - oracle corpus changes:
    - `test/neovm/vm-compat/cases/read-char-non-character-vector-tail-semantics.forms`
    - `test/neovm/vm-compat/cases/read-char-non-character-vector-tail-semantics.expected.tsv`
    - added deeper-tail probe `(list [1 2] 98 97 96 95 94)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-char-non-character-vector-tail-semantics` (pass, 6/6)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-char` non-character tail lock-ins with deeper payload depth (round 2):
  - oracle corpus changes:
    - `test/neovm/vm-compat/cases/read-char-non-character-tail-semantics.forms`
    - `test/neovm/vm-compat/cases/read-char-non-character-tail-semantics.expected.tsv`
    - added deeper-tail probe `(list 1.0 98 97 96 95 94)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-char-non-character-tail-semantics` (pass, 6/6)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-char` symbol tail lock-ins with deeper payload depth (round 2):
  - oracle corpus changes:
    - `test/neovm/vm-compat/cases/read-char-symbol-tail-semantics.forms`
    - `test/neovm/vm-compat/cases/read-char-symbol-tail-semantics.expected.tsv`
    - added deeper-tail probe `(list 'foo 98 'bar 'baz 'qux 'quux)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-char-symbol-tail-semantics` (pass, 6/6)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-char` vector-symbol tail lock-ins with deeper payload depth (round 2):
  - oracle corpus changes:
    - `test/neovm/vm-compat/cases/read-char-vector-symbol-tail-semantics.forms`
    - `test/neovm/vm-compat/cases/read-char-vector-symbol-tail-semantics.expected.tsv`
    - added deeper-tail probe `(list [foo] 98 'bar 'baz 'qux 'quux)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-char-vector-symbol-tail-semantics` (pass, 6/6)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-char` vector-vector tail lock-ins with deeper payload depth (round 2):
  - oracle corpus changes:
    - `test/neovm/vm-compat/cases/read-char-vector-vector-tail-semantics.forms`
    - `test/neovm/vm-compat/cases/read-char-vector-vector-tail-semantics.expected.tsv`
    - added deeper-tail probe `(list [foo] 98 [bar] [baz] [qux] [quux])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-char-vector-vector-tail-semantics` (pass, 6/6)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-key` vector/non-character-tail lock-ins with deeper payload depth (round 2):
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list [foo] 98 97 96 95 94 93 1.0)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-vector-vector-tail-non-character-tail-semantics` (pass, 9/9)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-key` vector/non-char tail lock-ins with deeper payload depth (round 2):
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list [foo] 104 103 102 101 100 99 98 97)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-vector-vector-tail-non-char-semantics` (pass, 9/9)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-key-sequence-vector` vector/non-char tail lock-ins with deeper payload depth (round 2):
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list [foo] 104 103 102 101 100 99 98 97)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-sequence-vector-vector-tail-non-char-semantics` (pass, 9/9)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-key-sequence-vector` vector-vector tail lock-ins with deeper payload depth (round 2):
  - oracle corpus changes:
  ...
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-sequence-vector-vector-tail-semantics` (pass, 9/9)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-key-sequence-vector` non-character tail lock-ins with deeper payload depth (round 2):
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 1.0 98 97 96 95 94 93 92)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-sequence-vector-non-character-tail-semantics` (pass, 9/9)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-key-sequence-vector` symbol-vector tail lock-ins with deeper payload depth (round 2):
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 'foo 98 [bar] [baz] [qux] [quux] [quuz] [corge])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-sequence-vector-symbol-vector-tail-semantics` (pass, 9/9)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-key-sequence` non-character tail lock-ins with deeper payload depth (round 2):
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 1.0 98 97 96 95 94 93 92)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-sequence-non-character-tail-semantics` (pass, 9/9)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-key-sequence` vector-symbol tail lock-ins with deeper payload depth (round 2):
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list [foo] 98 'bar 'baz 'qux 'quux 'quuz 'corge)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-sequence-vector-symbol-tail-semantics` (pass, 9/9)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-key-sequence` symbol tail lock-ins with deeper payload depth (round 2):
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 'foo 98 'bar 'baz 'qux 'quux 'quuz 'corge)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-sequence-symbol-tail-semantics` (pass, 9/9)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-key-sequence` vector tail lock-ins with deeper payload depth (round 2):
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 97 98 99 100 101 102 103 104 [mouse-1])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-sequence-vector-tail-semantics` (pass, 9/9)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-key` vector-vector tail lock-ins with deeper payload depth (round 2):
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list [foo] 98 [bar] [baz] [qux] [quux] [quuz] [corge])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-vector-vector-tail-semantics` (pass, 9/9)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-key` non-character tail lock-ins with deeper payload depth (round 2):
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 1.0 98 97 96 95 94 93 92)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-non-character-tail-semantics` (pass, 9/9)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-key` symbol-vector tail lock-ins with deeper payload depth (round 2):
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 'foo 98 [bar] [baz] [qux] [quux] [quuz] [corge])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-symbol-vector-tail-semantics` (pass, 9/9)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-key` vector-symbol tail lock-ins with deeper payload depth (round 2):
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 97 98 99 100 101 102 103 104 [foo])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-vector-symbol-tail-semantics` (pass, 9/9)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-key` symbol tail lock-ins with deeper payload depth (round 2):
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 'foo 98 'bar 'baz 'qux 'quux 'quuz)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-symbol-tail-semantics` (pass, 9/9)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-key` vector tail lock-ins with deeper payload depth (round 2):
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 97 98 99 100 101 102 103 104 [mouse-1])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-vector-tail-semantics` (pass, 9/9)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-event` list tail lock-ins with deeper payload depth (round 2):
  - oracle corpus changes:
  ...
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-event-list-tail-semantics` (pass, 10/10)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-event` non-character tail lock-ins with deeper payload depth (round 3):
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 1.0 98 97 96 95 94 93 92)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-event-non-character-tail-semantics` (pass, 9/9)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-event` vector tail lock-ins with deeper payload depth (round 3):
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 97 98 99 100 101 102 103 104 [mouse-1])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-event-vector-tail-semantics` (pass, 9/9)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-event` symbol tail lock-ins with deeper payload depth (round 3):
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 97 98 'foo 'bar 'baz 'qux 'quux 'quuz)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-event-symbol-tail-semantics` (pass, 9/9)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-event` symbol-vector tail lock-ins with deeper payload depth (round 3):
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 'foo 98 [bar] [baz] [qux] [quux] [quuz] [corge])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-event-symbol-vector-tail-semantics` (pass, 9/9)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-event` vector-symbol tail lock-ins with deeper payload depth (round 3):
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list [foo] 98 'bar 'baz 'qux 'quux 'quuz 'corge)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-event-vector-symbol-tail-semantics` (pass, 9/9)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-event` vector tail lock-ins with deeper payload depth (round 2):
  - oracle corpus changes:
    - `test/neovm/vm-compat/cases/read-event-vector-tail-semantics.forms`
    - `test/neovm/vm-compat/cases/read-event-vector-tail-semantics.expected.tsv`
    - added deeper-tail probe `(list 97 98 99 100 101 102 103 [mouse-1])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-event-vector-tail-semantics` (pass, 8/8)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-event` vector-vector tail lock-ins with deeper payload depth (round 2):
  - oracle corpus changes:
    - `test/neovm/vm-compat/cases/read-event-vector-vector-tail-semantics.forms`
    - `test/neovm/vm-compat/cases/read-event-vector-vector-tail-semantics.expected.tsv`
    - added deeper-tail probe `(list [foo] 98 [bar] [baz] [qux] [quux] [quuz])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-event-vector-vector-tail-semantics` (pass, 8/8)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-event` symbol-vector tail lock-ins with deeper payload depth (round 2):
  - oracle corpus changes:
    - `test/neovm/vm-compat/cases/read-event-symbol-vector-tail-semantics.forms`
    - `test/neovm/vm-compat/cases/read-event-symbol-vector-tail-semantics.expected.tsv`
    - added deeper-tail probe `(list 'foo 98 [bar] [baz] [qux] [quux] [quuz])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-event-symbol-vector-tail-semantics` (pass, 8/8)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-event` vector-symbol tail lock-ins with deeper payload depth (round 2):
  - oracle corpus changes:
    - `test/neovm/vm-compat/cases/read-event-vector-symbol-tail-semantics.forms`
    - `test/neovm/vm-compat/cases/read-event-vector-symbol-tail-semantics.expected.tsv`
    - added deeper-tail probe `(list [foo] 98 'bar 'baz 'qux 'quux 'quuz)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-event-vector-symbol-tail-semantics` (pass, 8/8)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-event` symbol tail lock-ins with deeper payload depth (round 2):
  - oracle corpus changes:
    - `test/neovm/vm-compat/cases/read-event-symbol-tail-semantics.forms`
    - `test/neovm/vm-compat/cases/read-event-symbol-tail-semantics.expected.tsv`
    - added deeper-tail probe `(list 97 98 'foo 'bar 'baz 'qux 'quux)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-event-symbol-tail-semantics` (pass, 8/8)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-event` non-character tail lock-ins with deeper payload depth (round 2):
  - oracle corpus changes:
    - `test/neovm/vm-compat/cases/read-event-non-character-tail-semantics.forms`
    - `test/neovm/vm-compat/cases/read-event-non-character-tail-semantics.expected.tsv`
    - added deeper-tail probe `(list 1.0 98 97 96 95 94 93)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-event-non-character-tail-semantics` (pass, 8/8)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-event` vector tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 97 98 99 100 101 102 [mouse-1])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-event-vector-tail-semantics` (pass, 7/7)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-event` list tail lock-ins with deeper payload depth:
  ...
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-event-list-tail-semantics` (pass, 9/9)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-event` vector-vector tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list [foo] 98 [bar] [baz] [qux] [quux])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-event-vector-vector-tail-semantics` (pass, 7/7)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-event` symbol-vector tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 'foo 98 [bar] [baz] [qux] [quux])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-event-symbol-vector-tail-semantics` (pass, 7/7)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-event` vector-symbol tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list [foo] 98 'bar 'baz 'qux 'quux)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-event-vector-symbol-tail-semantics` (pass, 7/7)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-event` symbol tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    - `test/neovm/vm-compat/cases/read-event-symbol-tail-semantics.forms`
    - `test/neovm/vm-compat/cases/read-event-symbol-tail-semantics.expected.tsv`
    - added deeper-tail probe `(list 97 98 'foo 'bar 'baz 'qux)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-event-symbol-tail-semantics` (pass, 7/7)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-event` non-character tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 1.0 98 97 96 95 94)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-event-non-character-tail-semantics` (pass, 7/7)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-event` vector tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 97 98 99 100 101 [mouse-1])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-event-vector-tail-semantics` (pass, 6/6)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-key-sequence-vector` list tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list '(mouse-1) 98 'foo 'bar 'baz 'qux 'quux 'quuz 'corge)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-sequence-vector-list-tail-semantics` (pass, 11/11)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-key-sequence` list tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list '(mouse-1) 98 'foo 'bar 'baz 'qux 'quux 'quuz 'corge)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-sequence-list-tail-semantics` (pass, 11/11)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-key` vector/vector non-char tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    - `test/neovm/vm-compat/cases/read-key-vector-vector-tail-non-char-semantics.forms`
    - `test/neovm/vm-compat/cases/read-key-vector-vector-tail-non-char-semantics.expected.tsv`
    - added deeper-tail probe `(list [foo] 103 102 101 100 99 98 97)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-vector-vector-tail-non-char-semantics` (pass, 8/8)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-key-sequence-vector` vector/vector non-char tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list [foo] 103 102 101 100 99 98 97)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-sequence-vector-vector-tail-non-char-semantics` (pass, 8/8)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-key-sequence-vector` non-character tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 1.0 98 97 96 95 94 93)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-sequence-vector-non-character-tail-semantics` (pass, 8/8)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-key-sequence-vector` vector-symbol-vector tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    - `test/neovm/vm-compat/cases/read-key-sequence-vector-symbol-vector-tail-semantics.forms`
    - `test/neovm/vm-compat/cases/read-key-sequence-vector-symbol-vector-tail-semantics.expected.tsv`
    - added deeper-tail probe `(list 'foo 98 [bar] [baz] [qux] [quux] [quuz])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-sequence-vector-symbol-vector-tail-semantics` (pass, 8/8)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-key-sequence-vector` vector-symbol tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    - `test/neovm/vm-compat/cases/read-key-sequence-vector-symbol-tail-semantics.forms`
    - `test/neovm/vm-compat/cases/read-key-sequence-vector-symbol-tail-semantics.expected.tsv`
    - added deeper-tail probe `(list [foo] 98 'bar 'baz 'qux 'quux 'quuz)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-sequence-vector-symbol-tail-semantics` (pass, 8/8)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-key` vector-symbol tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    - `test/neovm/vm-compat/cases/read-key-vector-symbol-tail-semantics.forms`
    - `test/neovm/vm-compat/cases/read-key-vector-symbol-tail-semantics.expected.tsv`
    - added deeper-tail probe `(list 97 98 99 100 101 102 103 [foo])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-vector-symbol-tail-semantics` (pass, 8/8)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-key-sequence-vector` vector/vector non-char tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    - `test/neovm/vm-compat/cases/read-key-sequence-vector-vector-tail-non-char-semantics.forms`
    - `test/neovm/vm-compat/cases/read-key-sequence-vector-vector-tail-non-char-semantics.expected.tsv`
    - added deeper-tail probe `(list [foo] 102 101 100 99 98 97)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-sequence-vector-vector-tail-non-char-semantics` (pass, 7/7)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-key-sequence` non-character tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 1.0 98 97 96 95 94 93)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-sequence-non-character-tail-semantics` (pass, 8/8)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-key` non-character tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 1.0 98 97 96 95 94 93)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-non-character-tail-semantics` (pass, 8/8)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-key-sequence-vector` vector-vector tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    - `test/neovm/vm-compat/cases/read-key-sequence-vector-vector-tail-semantics.forms`
    - `test/neovm/vm-compat/cases/read-key-sequence-vector-vector-tail-semantics.expected.tsv`
    - added deeper-tail probe `(list [foo] 98 [bar] [baz] [qux] [quux] [quuz])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-sequence-vector-vector-tail-semantics` (pass, 8/8)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-key` vector-vector tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list [foo] 98 [bar] [baz] [qux] [quux] [quuz])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-vector-vector-tail-semantics` (pass, 8/8)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-key-sequence` vector tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 97 98 99 100 101 102 103 [mouse-1])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-sequence-vector-tail-semantics` (pass, 8/8)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-key` vector tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 97 98 99 100 101 102 103 [mouse-1])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-vector-tail-semantics` (pass, 8/8)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-key-sequence-vector` vector/vector non-char tail lock-ins with deeper payload depth:
  ...
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-sequence-vector-vector-tail-non-char-semantics` (pass, 6/6)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-key-sequence` symbol tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 'foo 98 'bar 'baz 'qux 'quux 'quuz)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-sequence-symbol-tail-semantics` (pass, 8/8)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-key-sequence` symbol-vector tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 'foo 98 [bar] [baz] [qux] [quux] [quuz])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-sequence-symbol-vector-tail-semantics` (pass, 8/8)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-key` symbol-vector tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 'foo 98 [bar] [baz] [qux] [quux] [quuz])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-symbol-vector-tail-semantics` (pass, 8/8)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-key` symbol tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 'foo 98 'bar 'baz 'qux 'quux)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-symbol-tail-semantics` (pass, 8/8)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-key` vector-symbol tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    - `test/neovm/vm-compat/cases/read-key-vector-symbol-tail-semantics.forms`
    - `test/neovm/vm-compat/cases/read-key-vector-symbol-tail-semantics.expected.tsv`
    - added deeper-tail probe `(list 97 98 99 100 101 102 [foo])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-vector-symbol-tail-semantics` (pass, 7/7)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-key-sequence-vector` vector/vector non-char tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    - `test/neovm/vm-compat/cases/read-key-sequence-vector-vector-tail-non-char-semantics.forms`
    - `test/neovm/vm-compat/cases/read-key-sequence-vector-vector-tail-non-char-semantics.expected.tsv`
    - added deeper-tail probe `(list [foo] 100 99 98 97)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-sequence-vector-vector-tail-non-char-semantics` (pass, 5/5)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-key` vector/vector non-char tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list [foo] 100 99 98 97)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-vector-vector-tail-non-char-semantics` (pass, 5/5)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-key` vector tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 97 98 99 100 101 102 [mouse-1])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-vector-tail-semantics` (pass, 7/7)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-key` non-character tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 1.0 98 97 96 95 94)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-non-character-tail-semantics` (pass, 7/7)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-key-sequence` vector tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 97 98 99 100 101 102 [mouse-1])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-sequence-vector-tail-semantics` (pass, 7/7)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-key-sequence` non-character tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 1.0 98 97 96 95 94)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-sequence-non-character-tail-semantics` (pass, 7/7)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-key-sequence` symbol tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 'foo 98 'bar 'baz 'qux 'quux)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-sequence-symbol-tail-semantics` (pass, 7/7)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-key-sequence` list tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    - `test/neovm/vm-compat/cases/read-key-sequence-list-tail-semantics.forms`
    - `test/neovm/vm-compat/cases/read-key-sequence-list-tail-semantics.expected.tsv`
    - added deeper-tail probe `(list '(mouse-1) 98 'foo 'bar 'baz 'qux 'quux 'quuz)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-sequence-list-tail-semantics` (pass, 10/10)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-key-sequence-vector` symbol-vector tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 'foo 98 [bar] [baz] [qux] [quux])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-sequence-vector-symbol-vector-tail-semantics` (pass, 7/7)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-key-sequence-vector` non-character tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 1.0 98 97 96 95 94)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-sequence-vector-non-character-tail-semantics` (pass, 7/7)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)

- Expanded `read-key` vector/vector non-character tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    - `test/neovm/vm-compat/cases/read-key-vector-vector-tail-non-character-tail-semantics.forms`
    - `test/neovm/vm-compat/cases/read-key-vector-vector-tail-non-character-tail-semantics.expected.tsv`
    - added deeper-tail probe `(list [foo] 98 97 96 95 94 1.0)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-vector-vector-tail-non-character-tail-semantics` (pass, 8/8)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-key-sequence-vector` vector-vector tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list [foo] 98 [bar] [baz] [qux] [quux])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-sequence-vector-vector-tail-semantics` (pass, 7/7)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-key` vector-vector tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list [foo] 98 [bar] [baz] [qux] [quux])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-vector-vector-tail-semantics` (pass, 7/7)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-key-sequence` symbol-vector tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 'foo 98 [bar] [baz] [qux] [quux])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-sequence-symbol-vector-tail-semantics` (pass, 7/7)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-key` symbol-vector tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 'foo 98 [bar] [baz] [qux] [quux])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-symbol-vector-tail-semantics` (pass, 7/7)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-key-sequence` vector-symbol tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list [foo] 98 'bar 'baz 'qux 'quux)`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-sequence-vector-symbol-tail-semantics` (pass, 7/7)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-key` vector-symbol tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    - `test/neovm/vm-compat/cases/read-key-vector-symbol-tail-semantics.forms`
    - `test/neovm/vm-compat/cases/read-key-vector-symbol-tail-semantics.expected.tsv`
    - added deeper-tail probe `(list 97 98 99 100 101 [foo])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-vector-symbol-tail-semantics` (pass, 6/6)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded `read-key-sequence` vector tail lock-ins with deeper payload depth:
  - oracle corpus changes:
    ...
    - added deeper-tail probe `(list 97 98 99 100 101 [mouse-1])`.
  - verified:
    - `make -C test/neovm/vm-compat check-one-neovm CASE=cases/read-key-sequence-vector-tail-semantics` (pass, 6/6)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
...
- Expanded startup-variable `documentation-property` parity (round 31) with charset/coding/composition/compiler startup symbols:
  ...
  - verified:
    ...
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
- Expanded startup-variable `documentation-property` parity (round 32) with `current-*`, `dbus-*`, `debug-*`, and `default-*` startup symbols:
  - runtime changes:
    - added startup doc stubs for 30 additional symbols in `STARTUP_VARIABLE_DOC_STUBS`, including:
      - `current-iso639-language`, `current-key-remap-sequence`, `current-minibuffer-command`, `current-time-list`
      - `cursor-*` and `data-directory` startup vars
      - `dbus-*` message/version/registry vars
      - `debug-*` and debugger control vars
      - `default-frame-*`, `default-minibuffer-frame`, and `defining-kbd-macro`
    - integer `variable-documentation` lookups for that round-32 symbol batch now resolve through the shared startup doc stub mapping
    - added eval lock-in test:
      - `documentation_property_eval_debug_on_message_integer_property_returns_string`
  - expanded oracle corpus:
    - `test/neovm/vm-compat/cases/documentation-property-semantics.forms`
    - `test/neovm/vm-compat/cases/documentation-property-semantics.expected.tsv`
    - added a dedicated 30-symbol startup matrix row for the round-32 set (string doc + first-line prefix checks)
  - verified:
    - `cargo test --manifest-path rust/neovm-core/Cargo.toml documentation_property_eval_` (pass, 17/17)
    - `make -C test/neovm/vm-compat record FORMS=cases/documentation-property-semantics.forms EXPECTED=cases/documentation-property-semantics.expected.tsv` (pass)
...
- Expanded startup-variable `documentation-property` parity (round 37) with `inhibit-*` and startup input/env metadata symbols:
  ...
  - verified:
    ...
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
- Expanded startup-variable `documentation-property` parity (combined rounds 38-39) with input/internal/last-state and line/load metadata symbols:
  - runtime changes:
    - added startup doc stubs for 60 additional symbols in `STARTUP_VARIABLE_DOC_STUBS`, including:
      - round 38 input/internal and event-state vars: `input-pending-p-filter-events`, `installation-directory`, `integer-width`, `internal-*`, `interrupt-process-functions`, `inverse-video`, `invocation-*`, `kbd-macro-termination-hook`, `key-translation-map`, `keyboard-translate-table`, `kill-buffer-*`, `kill-emacs-hook`, `last-*` event/prefix vars
      - round 39 line/load/local vars: `last-repeatable-command`, `latin-extra-code-table`, `left-fringe-width`, `left-margin*`, `libgnutls-version`, `line-*`, `lisp-eval-depth-reserve`, `load-*` control vars, `local-*`, `locale-coding-system`, `long-line-*`, `lread--unescaped-character-literals`, `lucid--menu-grab-keyboard`
    - integer `variable-documentation` lookups for those 60 symbols now resolve through the shared startup doc stub mapping
    - added eval lock-in test:
      - `documentation_property_eval_load_suffixes_integer_property_returns_string`
  - expanded oracle corpus:
    - `test/neovm/vm-compat/cases/documentation-property-semantics.forms`
    - `test/neovm/vm-compat/cases/documentation-property-semantics.expected.tsv`
    - added dedicated startup-variable matrix rows for both 30-symbol batches (round 38 and round 39), asserting string docs and first-line prefixes
  - verified:
    - `cargo test --manifest-path rust/neovm-core/Cargo.toml documentation_property_eval_` (pass, 23/23)
    - `make -C test/neovm/vm-compat record FORMS=cases/documentation-property-semantics.forms EXPECTED=cases/documentation-property-semantics.expected.tsv` (pass)
    - `make -C test/neovm/vm-compat check-one-neovm CASE=documentation-property-semantics` (pass, 34/34)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
- Expanded startup-variable `documentation-property` parity (combined rounds 40-41) with macro/menu/minibuffer/mode-line/mouse/native-comp metadata symbols:
  - runtime changes:
    - added startup doc stubs for 60 additional symbols in `STARTUP_VARIABLE_DOC_STUBS`, including:
      - round 40 macro/menu/minibuffer control vars: `macroexp--dynvars`, `main-thread`, `make-*`, `mark-*`, `max-*`, `memory-*`, `menu-*`, `message-*`, `meta-prefix-char`, and the first minibuffer completion variables
      - round 41 minibuffer/mode-line/mouse/native vars: remaining `minibuffer-*` setup/history vars, `minor-mode-*`, `mode-line-*`, `mode-name`, `module-file-suffix`, `most-*fixnum`, `mouse-*`, `move-frame-functions`, `movemail-program-name`, `multibyte-syntax-as-symbol`, `multiple-frames`, `mwheel-coalesce-scroll-events`, `native-comp-eln-load-path`
    - integer `variable-documentation` lookups for those 60 symbols now resolve through the shared startup doc stub mapping
    - added eval lock-in test:
  ...
  - expanded oracle corpus:
    - `test/neovm/vm-compat/cases/documentation-property-semantics.forms`
    - `test/neovm/vm-compat/cases/documentation-property-semantics.expected.tsv`
    - added dedicated startup-variable matrix rows for both 30-symbol batches (round 40 and round 41), asserting string docs and first-line prefixes
  - verified:
    - `cargo test --manifest-path rust/neovm-core/Cargo.toml documentation_property_eval_` (pass, 24/24)
    - `make -C test/neovm/vm-compat record FORMS=cases/documentation-property-semantics.forms EXPECTED=cases/documentation-property-semantics.expected.tsv` (pass)
    - `make -C test/neovm/vm-compat check-one-neovm CASE=documentation-property-semantics` (pass, 36/36)
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
- Expanded startup-variable `documentation-property` parity (combined rounds 42-43) with native/network/overlay/post-command/print/process metadata symbols:
  - runtime changes:
    - added startup doc stubs for 60 additional symbols in `STARTUP_VARIABLE_DOC_STUBS`, including:
      - round 42 native/network/overlay parser vars: `native-comp-enable-subr-trampolines`, `native-comp-jit-compilation`, `network-coding-system-alist`, `next-screen-context-lines`, `no-redraw-on-reenter`, `nobreak-char-*`, `num-*`, `obarray`, `open-paren-in-column-0-is-defun-start`, `operating-system-release`, `otf-script-alist`, `other-window-scroll-*`, `overflow-newline-into-fringe`, `overlay-arrow-*`, `overline-margin`, `overriding-*`, `overwrite-mode`, `parse-sexp-*`, `path-separator`, `pdumper-fingerprint`
      - round 43 post-command/print/process vars: `point-before-scroll`, `polling-period`, `post-*`, `pre-*`, `prefix-*`, `preloaded-file-list`, `print-*`, `printable-chars`, `process-adaptive-read-buffering`, `process-coding-system-alist`, `process-connection-type`, `process-environment`, `process-error-pause-time`
    - integer `variable-documentation` lookups for those 60 symbols now resolve through the shared startup doc stub mapping
    - added eval lock-in test:
      - `documentation_property_eval_process_environment_integer_property_returns_string`
  - expanded oracle corpus:
    - `test/neovm/vm-compat/cases/documentation-property-semantics.forms`
    - `test/neovm/vm-compat/cases/documentation-property-semantics.expected.tsv`
    - added dedicated startup-variable matrix rows for both 30-symbol batches (round 42 and round 43), asserting string docs and first-line prefixes
  - verified:
    - `cargo test --manifest-path rust/neovm-core/Cargo.toml documentation_property_eval_` (pass, 25/25)
    - `make -C test/neovm/vm-compat record FORMS=cases/documentation-property-semantics.forms EXPECTED=cases/documentation-property-semantics.expected.tsv` (pass)
...
- Expanded startup-variable `documentation-property` parity (combined rounds 44-45) with process/read/redisplay/scroll/selection metadata symbols:
  ...
  - verified:
    ...
    - `make -C test/neovm/vm-compat check-all-neovm` (pass)
- Expanded startup-variable `documentation-property` parity (combined rounds 46-47) with signal/system/tab/tool/treesit metadata symbols:
  - runtime changes:
    - added startup doc stubs for 60 additional symbols in `STARTUP_VARIABLE_DOC_STUBS`, including:
      - round 46 signal/system/tab vars: `signal-process-functions`, `source-directory`, `special-event-map`, `standard-*`, `string*-consed`, `suspend-tty-functions`, `symbols-*`, `syntax-propertize--done`, `system-*`, `tab-bar-*`, `tab-line-format`, `tab-width`
      - round 47 text/tool/translation/treesit vars: `temp-buffer-show-function`, `temporary-file-directory`, `terminal-frame`, `text-*`, `this-*`, `throw-on-input`, `timer-*`, `tool-bar-*`, `tooltip-reuse-hidden-frame`, `top-level`, `track-mouse`, `translate-upper-case-key-bindings`, `translation-*`, `treesit-*`, `truncate-partial-width-windows`
    - integer `variable-documentation` lookups for those 60 symbols now resolve through the shared startup doc stub mapping
    - added eval lock-in test:
      - `documentation_property_eval_truncate_partial_width_windows_integer_property_returns_string`
  - expanded oracle corpus:
    - `test/neovm/vm-compat/cases/documentation-property-semantics.forms`
    - `test/neovm/vm-compat/cases/documentation-property-semantics.expected.tsv`
    - added dedicated startup-variable matrix rows for both 30-symbol batches (round 46 and round 47), asserting string docs and first-line prefixes
  - verified:
    - `cargo test --manifest-path rust/neovm-core/Cargo.toml documentation_property_eval_` (pass, 27/27)
    - `make -C test/neovm/vm-compat record FORMS=cases/documentation-property-semantics.forms EXPECTED=cases/documentation-property-semantics.expected.tsv` (pass)
