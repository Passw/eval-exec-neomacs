...
fn oracle_prop_format_width_and_padding_combinations() {
    ...
    assert_oracle_parity(r#"(format "[%-20s]" "hello world")"#);

    // Zero-padded integer
    assert_oracle_parity(r#"(format "[%010d]" 42)"#);
    assert_oracle_parity(r#"(format "[%010d]" -42)"#);
    assert_oracle_parity(r#"(format "[%06d]" 0)"#);
...
fn oracle_prop_format_float_precision() {
    ...
    // Basic precision
    assert_oracle_parity(r#"(format "%.2f" 3.14159265)"#);
    assert_oracle_parity(r#"(format "%.0f" 3.14159265)"#);
    assert_oracle_parity(r#"(format "%.10f" 3.14159265)"#);

    // Scientific notation with precision
    assert_oracle_parity(r#"(format "%.2e" 12345.6789)"#);
    assert_oracle_parity(r#"(format "%.10e" 1.0)"#);
    assert_oracle_parity(r#"(format "%.0e" 12345.6789)"#);

    // Width + precision combined
    assert_oracle_parity(r#"(format "[%12.4f]" 3.14159)"#);
    assert_oracle_parity(r#"(format "[%12.4f]" -3.14159)"#);
    assert_oracle_parity(r#"(format "[%-12.4f]" 3.14159)"#);

    // %g with precision
    assert_oracle_parity(r#"(format "%.2g" 0.00042)"#);
    assert_oracle_parity(r#"(format "%.6g" 123456.789)"#);
...
fn oracle_prop_format_S_vs_s_complex() {
    ...
    assert_oracle_parity(r#"(format "s=[%s] S=[%S]" "hello" "hello")"#);

    // Nested lists
    assert_oracle_parity(r#"(format "s=[%s] S=[%S]" '(1 "two" three) '(1 "two" three))"#);

    // nil and t
    assert_oracle_parity(r#"(format "s=%s S=%S s=%s S=%S" nil nil t t)"#);
    ...
    // Vectors
    assert_oracle_parity(r#"(format "s=%s S=%S" [1 2 3] [1 2 3])"#);
