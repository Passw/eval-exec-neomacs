...
fn file_size_human_readable_iec_formats_common_values() {
    ...
    for (input, expected) in cases {
        let value = dispatch_builtin(&mut eval, "file-size-human-readable-iec", vec![input])
...
fn fboundp_recognizes_dispatch_and_typed_builtin_names() {
    ...
    let with_temp_message = builtin_fboundp(&mut eval, vec![Value::symbol("with-temp-message")])
...
fn symbol_function_resolves_builtin_and_special_names() {
    ...
    let string_greater_alias = builtin_symbol_function(&mut eval, vec![Value::symbol("string>")])
        .expect("symbol-function should resolve string> alias");
    ...
    assert!(crate::emacs_core::autoload::is_autoload_value(
        &bookmark_delete
    ));
    for name in [
        "format-seconds",
        ...
        assert!(crate::emacs_core::autoload::is_autoload_value(&value));
    }
    for name in [
        "autoloadp",
        "seq-count",
...
fn indirect_function_resolves_builtin_and_special_names() {
    ...
    let string_less_alias = builtin_indirect_function(&mut eval, vec![Value::symbol("string<")])
        .expect("indirect-function should resolve string< alias");
    assert_eq!(string_less_alias, Value::Subr(intern("string-lessp")));

    let string_greater_alias = builtin_indirect_function(&mut eval, vec![Value::symbol("string>")])
        .expect("indirect-function should resolve string> alias");
    assert_eq!(string_greater_alias, Value::Subr(intern("string-greaterp")));

    let string_equal_alias = builtin_indirect_function(&mut eval, vec![Value::symbol("string=")])
        .expect("indirect-function should resolve string= alias");
