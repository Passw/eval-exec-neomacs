...
allowlist_file="${1:-$script_dir/cases/builtin-registry-commandp-allowlist.txt}"
...
awk '
  {
    gsub(/\\/, "\\\\", $0)
    gsub(/"/, "\\\"", $0)
    printf "(let* ((sym (intern \"%s\")) (ok (fboundp sym))) (if ok (commandp sym) :unbound))\n", $0
  }
...
echo "checked core commandp parity entries: $core_total"
echo "skipped extension entries: $extension_total"
echo "oracle/neovm commandp drifts: $drift"
...
echo "builtin registry commandp parity check passed"
